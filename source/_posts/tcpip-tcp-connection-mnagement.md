...
随便发了个HTTP请求，把数据包抓了一下，主要就是看一看三次握手的部分。 ![](https://www.dokyme.cn/wp-content/uploads/2017/07/Snipaste_2017-10-18_11-53-13.png) ![](https://www.dokyme.cn/wp-content/uploads/2017/07/Snipaste_2017-10-18_11-56-55.png) 从Options字段中可以看到： 1. 最大数据段长度是1460bytes。 2. 窗口缩放因子是8，也就是说把窗口大小左移8位。自此开始，所有相同方向的数据报文所表明的实际window size都是header的window size*2^factor。 3. 允许使用选择确认。 4. 有趣的是，这其中没有关于时间戳的选项。 下图是第一个GET请求。 ![](https://www.dokyme.cn/wp-content/uploads/2017/07/Snipaste_2017-10-18_12-03-08.png) 下图是从服务器向客户端发送的单向的数据包，服务器在握手阶段明确的mss是1394，于是可以看到后面的所有报文的长度最大为1448（1448-14(eth header)-20(ip header)-20(tcp header)=1394）。 ![](https://www.dokyme.cn/wp-content/uploads/2017/07/Snipaste_2017-10-18_12-08-11.png)

TCP状态转换
-------

一个TCP连接的每一端的状态都会在下面这张图中转换。有的转换是由于收到或者发送了包含状态位的报文而产生的，有的转换则是定时器超时导致。 ![](https://www.dokyme.cn/wp-content/uploads/2017/07/Snipaste_2017-10-18_13-55-51.png) 整个状态机围绕建立连接、传输数据、关闭连接这三大块进行组织。值得注意的是，椭圆形中的状态的英文名（SYN\_SENT、LISTEN、SYN\_RCVD）是基于unix、linux和windows系统中的`netstat`命令取的名字，而这个三个系统中的状态标识又是都基于一个RFC标准的描述。 ![](https://www.dokyme.cn/wp-content/uploads/2017/07/Snipaste_2017-10-18_13-59-58.png) 其实CLOSED并不是一个官方的状态，但是这里把CLOSED作为起点能够更加清晰明了地介绍TCP连接的状态转换关系。 图中有2点没有写出来的需要额外注明：
...
把三次握手和四次挥手的图与状态相结合，如下。 ![](https://www.dokyme.cn/wp-content/uploads/2017/07/Snipaste_2017-10-18_14-14-43.png)
...
![](https://www.dokyme.cn/wp-content/uploads/2017/07/Snipaste_2017-10-18_15-31-54.png) 主动关闭方在2MSL中收到了本应该在之前收到的报文，回复了ACK，但是此时恰好被动关闭方已经CLOSE，已经失去了所有关于此连接的信息，一无所知的被动关闭方收到了这个不明所以的ACK，只能回复一个RST，**而这个RST会导致主动关闭方提前CLOSE**。

TCP服务器选项
--------

在绝大多数运行的服务器上，服务器程序会监听某一个端口，当有新的连接到来时，建立一个新的进程或者线程来处理他，通常这个线程（进程）会启动一个socket。 以ssh为例，主进程监听22端口，当一个新的连接到来时，创建一个新的线程和socket，这个socket的状态为established。主进程继续监听。 ![](https://www.dokyme.cn/wp-content/uploads/2017/07/Snipaste_2017-10-21_19-23-04.png) 途中的本地ip地址和远程ip地址都是通配符，没有限制。
